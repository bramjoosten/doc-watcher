...
A local watcher script that mirrors a self-hosted Confluence (Data Center, or the EOL Server edition — same REST API) tree to disk so it can be searched and queried by IDE's and local AI. Authenticates with a user-level Personal Access Token, downloads in parallel with adaptive concurrency, and re-runs incrementally — every run re-enumerates the full subtree metadata, but only pages whose `version` has changed actually get re-fetched. **Confluence Cloud is out of scope**: it uses Basic-auth API tokens instead of Bearer PATs and serves the REST API under a `/wiki/` prefix, so the auth and path assumptions baked into `confluence.ts` don't apply.
...
- Config is a TypeScript module (`config.ts` at the project root, gitignored) that the user edits directly — no YAML parser, no `.env` file loaded. The schema in `src/config.ts` exports a `ConfigInput` type, and the user's `config.ts` ends with `} satisfies ConfigInput` so typos surface in the editor instead of at runtime. The user supplies `roots` as full Confluence URLs (any of: `/pages/viewpage.action?pageId=`, `/spaces/<KEY>/pages/<id>/`, `/display/<KEY>/<Title>`, `/display/<KEY>`, `/spaces/viewspace.action?key=<KEY>`); `base_url` is derived from the first root's origin and there is no separate id field. `loadConfig` warns if a stale `config.yaml` / `config.yml` / `config.json` sits next to `config.ts` — only the TS module is read. Selected keys can also be overridden by an environment variable so a single secret can be reused across multiple checkouts: the overlay is table-driven in `src/config.ts` (`ENV_OVERRIDES`), env wins over the file when non-empty, and each effective override is logged through `messages.config.envOverride`. Today only `DOC_WATCHER_PAT → pat` is wired; add a row to extend.
...
The index is the **source of truth** for all structural metadata: Confluence ID, version, ancestors, last-modified, links, embeds, webui URL. The `.md` files carry a small **regeneratable** YAML frontmatter view of the same data, so a single `.md` is self-describing without consulting the index — but the index always wins. Frontmatter fields are: `title`, `version`, `last_modified`, `last_modified_by`, `webui_url`. The frontmatter is rewritten from index data on every sync; nothing depends on the body of the frontmatter being canonical, so it can drift mid-run without consequence (the next sync overwrites it).
...
└── src/
    ├── taskmanager.ts              # entry point: sync with --includeNew / --reset flags
...
The user lists Confluence URLs in `roots` and `src/url-resolver.ts` turns each into a numeric page id at startup. The resolver tolerates the full self-hosted Confluence URL surface: trailing slashes are normalised away, then the path runs through a small ordered set of regexes. The shapes that resolve **without a network call**: any URL carrying a numeric `?pageId=<id>` (viewpage / editpage / copypage / movepage / draft action endpoints all qualify), `/spaces/<KEY>/pages/<id>[/<slug>]` (newer permalink), and `/pages/<id>` (flat permalink some DC builds use). The shapes that cost **one API call**: the pretty `/display/<KEY>/<Title>` form (`GET /rest/api/content?spaceKey=&title=&limit=1`) and every space-homepage flavour (`/display/<KEY>`, `/spaces/<KEY>`, `/spaces/<KEY>/overview`, `/spaces/<KEY>/pages`, `/spaces/viewspace.action?key=<KEY>`, all routing through `GET /rest/api/space/<KEY>?expand=homepage`). Confluence's `/x/<token>` Share-menu short links are followed with a manual `HEAD` (max 3 hops) and the destination URL is re-classified. Mixed origins across roots are rejected — auth, the adaptive limiter, and rate-budget headers are all origin-scoped, so a second origin would silently break those. The first URL's origin becomes the `baseUrl` everywhere downstream. Anything else (`/dashboard.action`, `/spaces/<KEY>/blog/...`, etc.) throws with a clear "supported shapes are..." message. **Cloud URLs (`/wiki/...`) are not supported** — see the project header for why.
...
**Default `sync` — CQL incremental.** One paginated call per root. When `state.last_sync` exists, the query carries a `lastmodified >=` filter, so the result is "just the pages edited since last run" — typically 1 request. On first run (or after `--reset` wiped state) the filter is omitted and CQL paginates the whole subtree. On a first run that comes back with ≤1 page, the sync emits a soft warning pointing at `--includeNew`: CQL goes through Confluence's Lucene index, which may not have materialised descendant relationships for a freshly-watched root yet (typically 1h for the indexer to catch up), and the DB walk would bypass that. The warning never falls back automatically — explicit opt-in stays the contract.
...
Auth: `Authorization: Bearer <pat>` on every request, where `pat` comes from `DOC_WATCHER_PAT` if set, otherwise from `config.ts`.
...
`npm start` is the only verb. Default behaviour: enumerate the subtree via CQL with the `lastmodified >=` filter (or no filter if no prior `last_sync`). With `--includeNew`, enumerate via the recursive `/child/page` DB walk instead (slower, but sees newly-created and deleted pages). With `--reset`, treat in-memory state as empty so every page in the enumeration looks new. Either way: diff each page's version against state, fetch any new-or-changed pages in parallel with `expand=body.storage,version,ancestors,space`, write both `.html` and `.md`, atomic state-file replace at the end. Incremental-mode runs print a hint at the bottom about `--includeNew` for new/deleted pages.

The `.html` saved next to each `.md` is the deterministic input that produced the markdown — kept on disk so a converter improvement can be applied to the existing corpus without re-fetching from Confluence, and so the raw source is available for debugging when the converter misbehaves.

Setup is manual: copy `config.example.ts` → `config.ts` and fill in the placeholders (`pat`, at least one `roots` URL). The shape is intentionally flat — every key sits at the top level of the default export. `roots` accepts either a single URL or a list of URLs; the URL resolver figures out the page id and derives `base_url` from the origin. The `satisfies ConfigInput` annotation at the bottom turns typos into compile-time errors.
