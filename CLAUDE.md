...
The user lists Confluence URLs in `roots` and `src/url-resolver.ts` turns each into a numeric page id at startup. Seven URL shapes are accepted: `/pages/viewpage.action?pageId=N`, `/spaces/<KEY>/pages/<id>/<slug>`, `/spaces/<KEY>/overview`, `/spaces/<KEY>` (bare), `/display/<KEY>/<Title>`, `/display/<KEY>`, and `/spaces/viewspace.action?key=<KEY>`. Shapes that already carry an id (`?pageId=` and `/spaces/<KEY>/pages/<id>/`) resolve without a network call, the pretty `/display/<KEY>/<Title>` form costs one `GET /rest/api/content?spaceKey=&title=&limit=1` lookup, and the four space-homepage flavours all funnel through `GET /rest/api/space/<KEY>?expand=homepage`. Mixed origins across roots are rejected — auth, the adaptive limiter, and rate-budget headers are all origin-scoped, so a second origin would silently break those. The first URL's origin becomes the `baseUrl` everywhere downstream.
